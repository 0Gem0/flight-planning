...
public class Graph {
        ...
        for(int v = idEnd; v!= -1 ; v = from[v] ){
            if (dist[idEnd] != Integer.MAX_VALUE) {
                for(String key : Graph.graph.keySet()){
                    if(Graph.graph.get(key).getId() == v){
                        order.add(key);
                    }
                }
            }
    ...
    public static void drawGraph(GraphicsContext graphicsContext,Place place,LinkedHashSet<Place> passed){
        ...
        for(Flight flight : place.getFlights()){
            ...
            graphicsContext.setFont(Font.font("Verdana", FontWeight.BOLD, 13));//Парметры текста для цен
            graphicsContext.setFill(Color.RED);
            graphicsContext.setLineWidth(3);
            double[] arrows = createArrow(place.getX(), place.getY(), flight.getFlightDest().getX(),flight.getFlightDest().getY(),10); //Создание стрелки
            graphicsContext.strokeLine(place.getX(), place.getY(), flight.getFlightDest().getX(),flight.getFlightDest().getY());//Рисовка пути(полета)
            graphicsContext.setStroke(Paint.valueOf("#ff0000"));//Цвет для стрелки
            graphicsContext.strokeLine(flight.getFlightDest().getX(),flight.getFlightDest().getY(), arrows[0], arrows[1]); // Рисование стрелки
            graphicsContext.strokeLine(flight.getFlightDest().getX(),flight.getFlightDest().getY(), arrows[2], arrows[3]);// Рисование стрелки
            graphicsContext.fillText(flight.getPrice()+ "",(int)((place.getX()+flight.getFlightDest().getX())/2 - 35),(int)((place.getY()+flight.getFlightDest().getY())/2 - 6)); // Рисовка цен
